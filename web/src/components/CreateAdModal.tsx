...
export function CreateAdModal() {
  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    if (weekDays.length <= 0 || !data.name || !data.discord || !data.game || !data.hourEnd || !data.hourStart || !data.yearsPlaying) {
  ...
  return (
    <Dialog.Portal>
      ...
      <Dialog.Content 
        className='fixed bg-[#2A2634] text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 py-8 px-10 rounded-lg w-[480px] shadow-lg shadow-black/25'
      >
        ...
        <form onSubmit={handleCreateAd} className='mt-8 flex flex-col gap-4'>
          ...
          <footer className='mt-4 flex justify-end gap-4'>
            <Dialog.Close
              type='button'
              className='bg-zinc-500 px-5 h-12 rounded-md font-semibold hover:opacity-70 transition-opacity'
            >
              Cancelar
            </Dialog.Close>
            <button
              type="submit"
              className='bg-violet-500 px-5 h-12 rounded-md font-semibold flex items-center gap-3 hover:opacity-70 transition-opacity'
            >
